<script>
  ...
  function get_cookie(){
    const cookie_data = new RegExp(`(?<=${"pmkey"}=)[^;]*`);
    return document.cookie.match(cookie_data)[0];
  }

  function send_postback(){
    const request_img = document.createElement('img');
    request_img.src = "https://postmarket.pro/api/cpa/postback?pmkey=" + get_cookie("pmkey") + "&hash=" + hash_url;
    document.cookie = "pmkey=" + 0;
  }; 

  // secretkey из личного кабинета рекламодателя
  const hash_url = MD5("5a94e4fb-58cb-4128-8224-8b3d46244823" + get_cookie());
  send_postback();
</script>